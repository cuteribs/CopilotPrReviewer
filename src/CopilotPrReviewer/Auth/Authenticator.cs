...
namespace Cuteribs.CopilotPrReviewer.Auth;
...
/// <summary>
/// OAuth-based authenticator using MSAL for OIDC
/// </summary>
...
public static class AuthenticatorFactory
{
    public static IAuthenticator CreateAuthenticator(string? authenticationType)
    {
        return (authenticationType ?? "").ToLowerInvariant() switch
        {
            ...
            "" => TryCreatePatAuthenticator() ?? (IAuthenticator)new OAuthAuthenticator(),
    ...
    private static PatAuthenticator CreatePatAuthenticator()
    {
        ...
        if (string.IsNullOrEmpty(pat))
        {
            throw new InvalidOperationException(
                "Personal Access Token (PAT) not found. Provide it via --pat argument or AZURE_DEVOPS_PAT environment variable.");
        }

        return new PatAuthenticator(pat);
    }

    private static PatAuthenticator? TryCreatePatAuthenticator()
    {
        var pat = Environment.GetEnvironmentVariable("AZURE_DEVOPS_PAT");
        return string.IsNullOrEmpty(pat) ? null : new PatAuthenticator(pat);
    }
}
