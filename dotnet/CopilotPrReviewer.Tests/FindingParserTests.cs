...
{
    private readonly CopilotReviewService _service;

    public FindingParserTests()
    {
        var settings = Options.Create(new CopilotSettings());
        var reviewSettings = Options.Create(new ReviewSettings());
        var guidelineProvider = new GuidelineProvider(reviewSettings);
        _service = new CopilotReviewService(settings, guidelineProvider, NullLogger<CopilotReviewService>.Instance);
    }
    ...
    public void Parse_ValidJsonArray_ReturnsFindings()
    {
        ...
        var findings = _service.ParseFindings(json);
    ...
    public void Parse_MarkdownWrappedJson_ReturnsFindings()
    {
        ...
        var findings = _service.ParseFindings(response);
    ...
    {
        var findings = _service.ParseFindings("[]");
    ...
    {
        var findings = _service.ParseFindings("not json at all");
        Assert.Empty(findings);
    }
    ...
    {
        var text = "Here:\n```\n[{\"a\":1}]\n```\nDone";
        var result = CopilotReviewService.ExtractJson(text);
    ...
    {
        var result = CopilotReviewService.ExtractJson("No JSON here at all");
        Assert.Null(result);
    }
}
